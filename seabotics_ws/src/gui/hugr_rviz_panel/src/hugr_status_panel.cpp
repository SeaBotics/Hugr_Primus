...
: rviz_common::Panel(parent)
{
  ...
  // --- Akselerasjon (fra IMU.linear_acceleration) ---
...
void HugrStatusPanel::onUiTimer()
{
  ...
  // Akselerasjon som én verdi (|a|)
  const double a_mag = std::sqrt(ax*ax + ay*ay + az*az);
  lbl_accel_->setText(QString::number(a_mag, 'f', 2));
...
QString raw_mode = mode_.trimmed().toLower();
raw_mode = raw_mode.simplified();
...
// MANUAL
else if (raw_mode == "manuell" ||
         raw_mode == "manual") {
    mode_text  = "Manual";
    mode_color = "#FFD700";   // Yellow
}

// AUTONOMOUS (robust against typos)
else if (raw_mode == "auto" ||
         raw_mode == "ato"  ||
         raw_mode == "aut") {
    mode_text  = "Autonomous";
    mode_color = "limegreen";   // Green
}
