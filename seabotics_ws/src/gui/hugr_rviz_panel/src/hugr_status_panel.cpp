...
: rviz_common::Panel(parent)
{
  ...
  grid->addWidget(new QLabel("Internal temperature (°C):"), r, 0);
  lbl_temp_ = new QLabel("0.0");
  lbl_temp_->setStyleSheet("color: blue;");
  grid->addWidget(lbl_temp_, r++, 1);

  grid->addWidget(new QLabel("Estimated runtime (min):"), r, 0);
  lbl_runtime_ = new QLabel("0.0");
  grid->addWidget(lbl_runtime_, r++, 1);
  ...
  lbl_mode_ = new QLabel("Kill Switch");
...
void HugrStatusPanel::startRos()
{
  ...
  sub_mode_ = node_->create_subscription<std_msgs::msg::Int8>(
    "/system_mode_status", 10,
  ...
  sub_battery_ = node_->create_subscription<sensor_msgs::msg::BatteryState>(
    "/power/state", 10,
    std::bind(&HugrStatusPanel::batteryCb, this, std::placeholders::_1));

  sub_runtime_ = node_->create_subscription<std_msgs::msg::Float32>(
    "/power/estimated_runtime_min", 10,
    std::bind(&HugrStatusPanel::runtimeCb, this, std::placeholders::_1));

  sub_temperature_ = node_->create_subscription<std_msgs::msg::Float32>(
    "/sensors/temperature", 10,
    std::bind(&HugrStatusPanel::temperatureCb, this, std::placeholders::_1));

  sub_odom_local_ = node_->create_subscription<nav_msgs::msg::Odometry>(
    "/odometry/filtered_local", 10,
    std::bind(&HugrStatusPanel::odomLocalCb, this, std::placeholders::_1));

  sub_odom_global_ = node_->create_subscription<nav_msgs::msg::Odometry>(
    "/odometry/filtered_global", 10,
    std::bind(&HugrStatusPanel::odomGlobalCb, this, std::placeholders::_1));
...
void HugrStatusPanel::modeCb(const std_msgs::msg::Int8::SharedPtr msg)
{
  std::lock_guard<std::mutex> l(mtx_);
  mode_ = msg->data;
}

void HugrStatusPanel::batteryCb(const sensor_msgs::msg::BatteryState::SharedPtr msg)
{
  std::lock_guard<std::mutex> l(mtx_);
  batt_ = msg->percentage * 100.0;
  battery_voltage_ = msg->voltage;
  battery_current_ = msg->current;
}

void HugrStatusPanel::runtimeCb(const std_msgs::msg::Float32::SharedPtr msg)
{
  std::lock_guard<std::mutex> l(mtx_);
  runtime_min_ = msg->data;
}

void HugrStatusPanel::temperatureCb(const std_msgs::msg::Float32::SharedPtr msg)
{
  std::lock_guard<std::mutex> l(mtx_);
  temp_ = msg->data;
}

void HugrStatusPanel::odomLocalCb(const nav_msgs::msg::Odometry::SharedPtr msg)
{
  std::lock_guard<std::mutex> l(mtx_);

  const auto & v = msg->twist.twist.linear;
  speed_ = std::sqrt(v.x * v.x + v.y * v.y + v.z * v.z);
}

void HugrStatusPanel::odomGlobalCb(const nav_msgs::msg::Odometry::SharedPtr msg)
{
  std::lock_guard<std::mutex> l(mtx_);
...
{
  double roll, pitch, yaw, ax, speed, batt, temp, runtime_min, x, y, z;
  int mode;
  ...
  if (lbl_runtime_) lbl_runtime_->setText(fmt(runtime_min, 1));
  if (lbl_speed_) lbl_speed_->setText(fmt(speed, 2));
  if (lbl_accel_) lbl_accel_->setText(fmt(ax, 2));

  if (lbl_heading_) {
    lbl_heading_->setText(fmt(std::fmod((yaw * 180.0 / M_PI + 90.0 + 360.0), 360.0), 1));
  }
